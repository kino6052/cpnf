...
```javascript
function twinSiftingProcess():
  B = {2, 3}                     // Initial blockers
  T = empty set                  // Certified twin-indices
  k = 0

  while true:
    y_k = max(B)                 // Largest known prime
    W_k = floor((y_k^2 - 1)/6)   // Certification window bound

    // Find new twin-index in window
    newTwinIndex = null
    for n = 1 to W_k:
      if n not in T and isTwinCandidate(n, B):
        newTwinIndex = n
        break

    if newTwinIndex != null:
      T = T union {newTwinIndex} // Certify twin-index
      // Add larger prime of the pair as new blocker
      p = max(6n-1, 6n+1)
      B = B union {p}
    else:
      // No twin certified this stage - find next ordinary prime
      nextPrime = findMinimalSurvivor(B)
      B = B union {nextPrime}
...
- `isTwinCandidate(n, B)`: Returns true if both 6n−1 and 6n+1 survive division by all p ∈ B
- `findMinimalSurvivor(B)`: Finds the smallest integer greater than 1 not divisible by any p ∈ B (ordinary CPNF)
